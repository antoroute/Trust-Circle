...
for role_name in admin migrator auth messaging; do
  openssl rand -hex 32 >"$tc201_logs/${role_name}_db_password"
done
chmod 0444 "$tc201_logs"/*_db_password
test "$(sort -u "$tc201_logs"/*_db_password | wc -l)" -eq 4
export TC_MIGRATION_TEST_ADMIN_PASSWORD_FILE="$tc201_logs/admin_db_password"
export TC_MIGRATION_TEST_MIGRATOR_PASSWORD_FILE="$tc201_logs/migrator_db_password"
export TC_MIGRATION_TEST_AUTH_PASSWORD_FILE="$tc201_logs/auth_db_password"
export TC_MIGRATION_TEST_MESSAGING_PASSWORD_FILE="$tc201_logs/messaging_db_password"
...
tc201_finish() {
  ...
  if [[ "$test_status" -ne 0 ]]; then
    "${tc201_compose[@]}" logs --no-color --tail 200 auth messaging >&2 || true
  fi
...
"${tc201_compose[@]}" up --no-color bootstrap_roles
test "$("${tc201_compose[@]}" ps --all --quiet bootstrap_roles | xargs docker inspect --format '{{.State.ExitCode}}')" = 0
"${tc201_compose[@]}" run --rm sqitch --version \
  | grep -q 'sqitch (App::Sqitch) v1\.6\.1$'
"${tc201_compose[@]}" exec -T database \
  psql --tuples-only --no-align --username trust_circle_admin \
...
"${tc201_compose[@]}" exec -T database \
  psql --set ON_ERROR_STOP=1 --username trust_circle_admin \
  --dbname trust_circle_migration_test \
  <"$tc201_dir/tests/assert-current-schema.sql"
"${tc201_compose[@]}" exec -T database \
  psql --set ON_ERROR_STOP=1 --username trust_circle_admin \
  --dbname trust_circle_migration_test \
  <"$tc201_dir/tests/assert-role-separation.sql"
...
"${tc201_compose[@]}" exec -T database \
  ...
  --username trust_circle_admin --dbname trust_circle_migration_test \
