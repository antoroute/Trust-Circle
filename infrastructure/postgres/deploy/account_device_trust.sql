-- Deploy trust-circle:account_device_trust to pg
-- requires: unique_pending_join_request
...
CREATE TABLE account_devices (
  ...
  platform TEXT NOT NULL
    CHECK (platform IN ('android', 'ios', 'windows', 'macos', 'unknown')),
...
CREATE TABLE device_registration_challenges (
  ...
  bootstrap_grant_id UUID
    REFERENCES device_bootstrap_grants(id) ON DELETE SET NULL,
  expires_at TIMESTAMPTZ NOT NULL,
  consumed_at TIMESTAMPTZ,
  result TEXT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
